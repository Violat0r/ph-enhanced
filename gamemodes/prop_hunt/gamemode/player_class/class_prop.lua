...
function CLASS:OnSpawn(pl)
	...
	if GetConVar("ph_use_custom_plmodel_for_prop"):GetBool() then
		if table.HasValue(PROP_PLMODEL_BANS, player_manager.TranslatePlayerModel(pl:GetInfo("cl_playermodel"))) then
			pl.ph_prop:SetModel("models/player/kleiner.mdl")
			pl:ChatPrint("Your custom playermodel was banned from Props.")
		elseif table.HasValue(PROP_PLMODEL_BANS, pl:GetInfo("cl_playermodel")) then
			pl.ph_prop:SetModel("models/player/kleiner.mdl")
			pl:ChatPrint("Your custom playermodel was banned from Props.")
		else
